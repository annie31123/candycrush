...
int chocolate::condiction(basic *a[Row][Col], basic *focus)
{
    ...
    if(num!=0&&num!=50){
        ...
        if(num/10==0){
            ...
            if(j<Col-2&&j>1&&num%10==a[i][j+2]->color%10 && num==a[i][j+1]->color%10 &&num%10==a[i][j-1]->color%10&&num%10==a[i][j-2]->color%10)
                return 3;
            if(i<Row-2&&i>1&& num%10==a[i+1][j]->color%10 &&  num%10==a[i+2][j]->color%10 & num%10==a[i-1][j]->color%10&& num%10==a[i-2][j]->color%10)
                return 4;
        }
...
void chocolate::make(basic *a[Row][Col], basic *focus,int mode)
{
    ...
    case 1: //水平
        ...
        fivenum[2]=a[i][j]->color;
        if( a[i][j+1]->color/10!=0) { fivenum[3]=a[i][j+1]->color/10;}
        if( a[i][j+2]->color/10!=0) { fivenum[4]=a[i][j+2]->color/10;}
    ...
    case 2:
        ...
        if( a[i+2][j]->color/10!=0) { fivenum[4]=a[i+2][j]->color/10;}
